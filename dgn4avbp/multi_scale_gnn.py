...
class MultiScaleGnn(nn.Module):
    ...
    class DownBlock(nn.Module):
        ...
        def forward(self,
                    ...
                    v: torch.Tensor,
                    e: torch.Tensor,
                    emb: torch.Tensor) -> tuple[Data, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
            """Forward pass through the down block. 

            Args:
                graph (Data): Data. The `edge_index` and `batch` attributes will be modified.
                v (torch.Tensor): Node features. Dimensions: (num_nodes, mlps_width).
                e (torch.Tensor): Edge features. Dimensions: (num_edges, mlps_width).
                emb (torch.Tensor): Diffusion-step embedding. Dimensions: (batch_size, emb_width).
        
            Returns:
                Data: Lower resolution graph. The `edge_index` and `batch` attributes are modified.
    ...
    class UpBlock(nn.Module):
        ...
        def forward(
            ...
            emb: torch.Tensor,
        ) -> tuple[torch.Tensor, torch.Tensor, Data]:
            ...
            Returns:
                Data: Higher resolution graph. The `edge_index` and `batch` attributes are modified.
